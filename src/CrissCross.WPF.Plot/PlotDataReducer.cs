...
public static class PlotDataReducer
{
    ...
    public static PlotSeriesData LargestTriangleThreeBuckets(
        ...
        int targetPointCount)
    {
        ...
        var bucketSize =
            (series.X.Count - EndpointCount) / (double)(targetPointCount - EndpointCount);
        var selectedIndex = 0;
        for (var bucket = 0; bucket < targetPointCount - EndpointCount; bucket++)
        {
            ...
            selectedIndex = SelectLargestTrianglePoint(
                series,
                bucket,
                bucketSize,
                selectedIndex,
        ...
        double bucketSize)
    {
        var averageStart = Math.Min(
            series.X.Count - 1,
            (int)Math.Floor((bucket + 1) * bucketSize) + 1);
        var averageEnd = Math.Min(
            series.X.Count,
            (int)Math.Floor((bucket + EndpointCount) * bucketSize) + 1);
        ...
        (double X, double Y) average)
    {
        var rangeStart = Math.Min(
            series.X.Count - EndpointCount,
            (int)Math.Floor(bucket * bucketSize) + 1);
        ...
        {
            var area = Math.Abs(
                ((pointAx - average.X) * (series.Y[i] - pointAy))
                    - ((pointAx - series.X[i]) * (average.Y - pointAy)));
            if (area <= maximumArea)
            {
                continue;
            }
