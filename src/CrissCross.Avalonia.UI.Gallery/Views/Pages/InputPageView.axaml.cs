...
public partial class InputPageView : ReactiveUserControl<InputPageViewModel>
{
    ...
    public InputPageView()
    {
        ...
        if (DemoRichTextBox is { } richTextBox)
        {
            LoadRepresentativeRichTextContent(richTextBox);
        }
    }

    private RichTextBoxControl? DemoRichTextBox => this.FindControl<RichTextBoxControl>("FormattingRichTextBox");

    private static void EnsureDemoSelection(RichTextBoxControl richTextBox)
    {
        if (richTextBox.HasSelection || richTextBox.GetPlainText().Length == 0)
        {
            return;
        }

        richTextBox.Select(0, Math.Min(12, richTextBox.GetPlainText().Length));
    }
    ...
    private void OnRichTextInsertImageClick(object? sender, RoutedEventArgs e)
    {
        ...
        richTextBox.AppendHtml($"<p>Inline image inserted from a data URI:</p><img src='{DemoImageDataUri}' width='48' height='48' align='left' /><p></p>");
    ...
    private void OnRichTextFontSizeClick(object? sender, RoutedEventArgs e) => ApplySelectionAction(richTextBox => richTextBox.SetSelectionFontSize(20), "Font Size changed to 20.");
    ...
    {
        var html = DemoRichTextBox?.GetHtml() ?? string.Empty;
    ...
    {
        var markdown = DemoRichTextBox?.GetMarkdown() ?? string.Empty;
    ...
    private void UpdateDisplayPreview(string status)
    {
        ...
        if (source is not null && display is not null)
        {
            display.SetHtml(source.GetHtml());
            display.Select(0, 0);
        }
    ...
    {
        var richTextBox = DemoRichTextBox;
        var plainLength = richTextBox?.GetPlainText().Length ?? 0;
        var htmlLength = richTextBox?.GetHtml().Length ?? 0;
