...
public class BidirectionalNavigationRegistryTests
{
    ...
    public async Task NavigateView_WithSuppliedView_PreservesViewAndCompatibleViewModel()
    {
        ...
        var result = await registry
            .CreateNavigator()
            .NavigateView(new ViewNavigationRequest<CustomerPageViewModel, CustomerPageView> { View = expectedView })
            .FirstAsync();
    ...
    public async Task NavigateView_WithNullNavigator_ThrowsArgumentNullException()
    {
        ...
        await Assert
            .That(() => navigator!.NavigateView(new NavigationKeyRequest<ICustomerPageView>()))
    ...
    public async Task NavigateViewModel_WithNullNavigator_ThrowsArgumentNullException()
    {
        ...
        await Assert
            .That(() => navigator!.NavigateViewModel(new NavigationKeyRequest<ICustomerPageViewModel>()))
            .Throws<ArgumentNullException>();
    }
    ...
    public async Task Register_ViewModelAndView_AllowsViewModelInstanceNavigationToResolveView()
    {
        ...
        var result = await registry
            .CreateNavigator()
            .NavigateViewModel(
                new ViewModelNavigationRequest<CustomerPageViewModel, CustomerPageView> { ViewModel = expectedViewModel, })
            .FirstAsync();
    ...
    public async Task Register_InterfaceKeys_AllowsViewModelInterfaceNavigationToResolveConcreteView()
    {
        ...
        _ = registry.Register(
            new NavigationRegistration<
                ...
                ICustomerPageView,
                CustomerPageView
            >(static _ => new CustomerPageViewModel(), static _ => new CustomerPageView()));

        var result = await registry
            .CreateNavigator()
            .NavigateViewModel(new NavigationKeyRequest<ICustomerPageViewModel>())
            .FirstAsync();
    ...
    public async Task Register_InterfaceKeys_PreservesConcreteIdentityContractAndParameter()
    {
        ...
        _ = registry.Register(
            new NavigationRegistration<
                ...
                ICustomerPageView,
                CustomerPageView
            >(_ => expectedViewModel, _ => expectedView)
            { Contract = DetailContract, });

        var result = await registry
            .CreateNavigator()
            .NavigateViewModel(
                new NavigationKeyRequest<ICustomerPageViewModel> { Options = new NavigationRequestOptions { Contract = DetailContract, Parameter = expectedParameter }, })
            .FirstAsync();
    ...
    public async Task Register_ViewInterfaceKey_AllowsViewInterfaceNavigationToResolveConcreteViewModel()
    {
        ...
        var result = await registry
            .CreateNavigator()
            .NavigateView(new NavigationKeyRequest<ICustomerPageView>())
            .FirstAsync();
    ...
    public async Task NavigateView_RuntimeInterfaceKey_ResolvesConcreteViewAndViewModel()
    {
        ...
        var result = await registry
            ...
            .NavigateView(
                viewKey,
                new NavigationRequestOptions { Contract = DetailContract, Parameter = expectedParameter })
            .FirstAsync();
    ...
    public async Task Register_SameViewModelWithTwoContracts_ResolvesContractSpecificView()
    {
        ...
        var summary = await registry
            .CreateNavigator()
            .NavigateViewModel(
                new ViewModelNavigationRequest<CustomerPageViewModel, CustomerSummaryView> { Options = new NavigationRequestOptions { Contract = SummaryContract }, })
            .FirstAsync();
        var detail = await registry
            .CreateNavigator()
            .NavigateViewModel(
                new ViewModelNavigationRequest<CustomerPageViewModel, CustomerDetailView> { Options = new NavigationRequestOptions { Contract = DetailContract }, })
            .FirstAsync();
    ...
    public async Task Register_SameViewWithTwoContracts_ResolvesContractSpecificViewModel()
    {
        ...
        var summary = await registry
            .CreateNavigator()
            .NavigateView(
                new ViewNavigationRequest<CustomerSummaryViewModel, CustomerSummaryView> { Options = new NavigationRequestOptions { Contract = SummaryContract }, })
            .FirstAsync();
        var detail = await registry
            .CreateNavigator()
            .NavigateView(
                new ViewNavigationRequest<CustomerReadOnlyViewModel, CustomerReadOnlyView> { Options = new NavigationRequestOptions { Contract = DetailContract }, })
            .FirstAsync();
    ...
    public async Task Navigate_WithUnknownContract_ThrowsNavigationResolutionExceptionWithKnownContracts()
    {
        ...
        var exception = await CaptureNavigationResolutionException(() =>
            registry
                .CreateNavigator()
                .NavigateViewModel(
                    new ViewModelNavigationRequest<CustomerPageViewModel, CustomerSummaryView> { Options = new NavigationRequestOptions { Contract = DetailContract }, })
                .FirstAsync());
        ...
        await Assert.That(ContainsContract(exception.KnownContracts, SummaryContract)).IsTrue();
    }
    ...
    public async Task Navigate_CancellationBeforeResolution_DoesNotInvokeFactories()
    {
        ...
        var exception = await CaptureOperationCanceledException(() =>
            registry
                .CreateNavigator()
                .NavigateViewModel(
    ...
    /// <param name="action">The action value.</param>
    /// <returns>The result.</returns>
    private static async Task<NavigationResolutionException> CaptureNavigationResolutionException(
        Func<Task<NavigationResolution<CustomerPageViewModel, CustomerSummaryView>>> action)
    {
        try
        {
            _ = await action();
    ...
    /// <param name="action">The action value.</param>
    /// <returns>The result.</returns>
    private static async Task<OperationCanceledException> CaptureOperationCanceledException(
        Func<Task<NavigationResolution<CustomerPageViewModel, CustomerPageView>>> action)
    {
        try
        {
            _ = await action();
    ...
    /// <summary>Checks whether a navigation contract is present without an iterator allocation.</summary>
    /// <param name="contracts">The contracts to inspect.</param>
    /// <param name="expectedContract">The contract to find.</param>
    /// <returns><see langword="true"/> when the contract is present; otherwise <see langword="false"/>.</returns>
    private static bool ContainsContract(IEnumerable<string?> contracts, string expectedContract)
    {
        foreach (var contract in contracts)
        {
            if (string.Equals(contract, expectedContract, StringComparison.Ordinal))
            {
                return true;
            }
        }
