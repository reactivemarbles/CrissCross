...
internal static class GifHelpers
{
    ...
    internal static async Task<string> ReadStringAsync(Stream stream, int length)
    {
    ...
    /// <returns>The result.</returns>
    internal static Task ConsumeDataBlocksAsync(
        Stream sourceStream,
        CancellationToken cancellationToken = default) =>
        CopyDataBlocksToStreamAsync(sourceStream, Stream.Null, cancellationToken);

    /// <summary>Provides the ReadDataBlocksAsync member.</summary>
    /// <param name="stream">The stream value.</param>
    /// <param name="cancellationToken">The cancellationToken value.</param>
    /// <returns>The result.</returns>
    internal static async Task<byte[]> ReadDataBlocksAsync(Stream stream, CancellationToken cancellationToken = default)
    {
    ...
    /// <param name="cancellationToken">The cancellationToken value.</param>
    /// <returns>The result.</returns>
    internal static async Task CopyDataBlocksToStreamAsync(
        Stream sourceStream,
        Stream targetStream,
        CancellationToken cancellationToken = default)
    {
    ...
    /// <returns>The result.</returns>
    internal static async Task<GifColor[]> ReadColorTableAsync(Stream stream, int size)
    {
    ...
    /// <returns>The result.</returns>
    internal static bool IsNetscapeExtension(GifApplicationExtension ext) =>
    ...
    /// <param name="ext">The ext value.</param>
    /// <returns>The result.</returns>
    internal static ushort GetRepeatCount(GifApplicationExtension ext) => ext.Data is { Length: >= NetscapeRepeatCountDataLength } data
    ...
    internal static Exception UnknownBlockTypeException(int blockId) =>
        new UnknownBlockTypeException($"Unknown block type: 0x{blockId:x2}");
    ...
    internal static Exception UnknownExtensionTypeException(int extensionLabel) =>
        new UnknownExtensionTypeException($"Unknown extension type: 0x{extensionLabel:x2}");
    ...
    /// <returns>The result.</returns>
    internal static Exception InvalidSignatureException(string signature) =>
    ...
    /// <returns>The result.</returns>
    internal static Exception UnsupportedVersionException(string version) =>
    ...
    /// <returns>The result.</returns>
    internal static string GetString(byte[]? bytes) => bytes is null ? string.Empty : GetString(bytes, 0, bytes.Length);
    ...
    /// <returns>The result.</returns>
    internal static string GetString(byte[] bytes, int index, int count) => Encoding.UTF8.GetString(bytes, index, count);
}
