...
public class SymbolIcon : FontIcon
{
    ...
    static SymbolIcon()
    {
        _ = SymbolProperty.Changed.AddClassHandler<SymbolIcon>(static (x, _) => x.OnGlyphChanged());
        _ = FilledProperty.Changed.AddClassHandler<SymbolIcon>(static (x, _) => x.OnFilledChanged());
    }
