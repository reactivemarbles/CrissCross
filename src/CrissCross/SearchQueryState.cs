    ...
    IReadOnlyList<FilterToken>? filters = null)
{
    ...
    /// <summary>Gets the latest debounced search text.</summary>
    public string? DebouncedText { get; } = debouncedText;

    /// <summary>Gets the latest explicitly submitted search text.</summary>
    public string? SubmittedText { get; } = submittedText;

    /// <summary>Gets a value indicating whether a search operation is active.</summary>
    public bool IsSearching { get; } = isSearching;

    /// <summary>Gets the current result count, when known.</summary>
    public int? ResultCount { get; } = resultCount;

    /// <summary>Gets the configured filter tokens.</summary>
    public IReadOnlyList<FilterToken>? Filters { get; } = filters;

    /// <summary>Gets the trimmed current query text.</summary>
    public string NormalizedText => (Text ?? string.Empty).Trim();

    /// <summary>Gets a value indicating whether the state has current query text.</summary>
    public bool HasQuery => NormalizedText.Length > 0;

    /// <summary>Gets the active filter tokens.</summary>
    public IReadOnlyList<FilterToken> ActiveFilters => Filters ?? [];

    /// <summary>Gets the number of active filter tokens.</summary>
    public int ActiveFilterCount => ActiveFilters.Count;

    /// <summary>Gets a value indicating whether active filters are present.</summary>
    public bool IsFiltered => ActiveFilterCount > 0;

    /// <summary>Gets compact user-facing result count text.</summary>
